...
-- questo da' errore -> subquery non puo' restituire piu' di un valore quando l'operatore e' tra questi  =, !=, <, <= , >, >=
SELECT orderid
FROM Sales.Orders
WHERE empid = 
  (SELECT E.empid
...
-- questo non da' errore -> l'operatore e' "in"  (vedi: Multi-Valued Subqueries)
SELECT orderid
FROM Sales.Orders
WHERE empid in
  (SELECT E.empid
